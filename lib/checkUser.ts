...
type PlanType = "pro" | "starter" | "free";
...
const getCurrentPlan = async (): Promise<PlanType> => {
...
const shouldAllocateCredits = (dbUser: User, currentPlan: PlanType): boolean => {
...
export const checkUser = async (): Promise<User | null> => {
    ...
    try {
        ...
        if (!email) {
            console.error("checkUser error: user has no email address");
            return null;
        }
        ...
        console.error("checkUser error - FULL:", JSON.stringify(error, null, 2));
        console.error("checkUser error - RAW:", error);
        return null;
    }
};