...
import type { IntervieweeAppointment } from "@/actions/appointments";

/** The subset of User fields this card needs for either party on a booking. */
interface PersonSummary {
    ...
    title: string | null;
    company: string | null;
    /** Interviewer-only field; absent on interviewee summaries. */
    categories?: InterviewCategory[];
}

/**
 * Booking shape from the interviewer's side (includes `interviewee` instead
 * of `interviewer`). Mirrors `IntervieweeAppointment` from
 * `@/actions/appointments` — adjust if the real interviewer-facing query
 * (e.g. `getInterviewerAppointments`) differs.
 */
interface InterviewerAppointment {
    id: string;
    startTime: Date;
    endTime: Date;
    ...
    interviewee: PersonSummary | null;
}

type AppointmentCardBooking = IntervieweeAppointment | InterviewerAppointment;
...
export function AppointmentCard({
    ...
    const person: PersonSummary | null =
        mode === "interviewer"
            ? (booking as InterviewerAppointment).interviewee
            : (booking as IntervieweeAppointment).interviewer;
    ...
    return (
        <>
            <FeedbackModal
                ...
                intervieweeName={
                    mode === "interviewer"
                        ? (booking as InterviewerAppointment).interviewee?.name
                        : undefined
                }
            />
