...
type PaymentMethodValue = "PAYPAL" | "BANK" | "UPI";
...
/**
 * Mirrors the shape returned by `getInterviewerStats` in
 * `@/actions/dashboard`.
 */
...
/**
 * Mirrors the shape returned by `getWithdrawalHistory` in
 * `@/actions/dashboard` (a `db.payout` record).
 */
interface Payout {
    id: string;
    credits: number;
...
export default function EarningsSection({ stats, history }: EarningsSectionProps) {
    const [open, setOpen] = useState(false);
    const [done, setDone] = useState(false);
    const [method, setMethod] = useState<PaymentMethodValue>("PAYPAL");
    ...
    const {
        ...
        [{ credits: number; paymentMethod: PaymentMethodValue; paymentDetail: string }]
    >(requestWithdrawal);
                            ...
                            <div className="flex flex-col gap-5 py-2">
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    <Tabs
                                        value={method}
                                        onValueChange={(val) => {
                                            setMethod(val as PaymentMethodValue);
                                            setDetail("");
                                        }}
                                    >
