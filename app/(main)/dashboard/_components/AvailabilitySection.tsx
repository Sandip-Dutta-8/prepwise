...
import { setAvailability, deleteAvailability } from "@/actions/dashboard";
...
interface SetAvailabilityResult {
    success: boolean;
}

interface DeleteAvailabilityResult {
    success: boolean;
}

interface AvailabilitySectionProps {
    initial: AvailabilityWindow | null;
}
...
export default function AvailabilitySection({
    ...
    // Tracks whether an availability window currently exists, independent of
    // the `initial` prop — so the UI updates immediately after a delete/save
    // without waiting on the parent to re-render with fresh server data.
    ...
    const {
        ...
        error,
        fn: saveFn,
    } = useFetch<SetAvailabilityResult, [{ startTime: string; endTime: string }]>(
        setAvailability
    );
    ...
    const {
        ...
        error: deleteError,
        fn: deleteFn,
    } = useFetch<DeleteAvailabilityResult, []>(deleteAvailability);
    ...
    useEffect(() => {
        if (deleteData?.success) {
            ...
            setStartTime("");
            setEndTime("");
            const t = setTimeout(() => setDeleted(false), 3000);
            return () => clearTimeout(t);
        }
    }, [deleteData]);
