...
type AssignableRole = Exclude<UserRole, "UNASSIGNED">;
...
export default function OnboardingPage() {
    ...
    const toggleCategory = (val: InterviewCategory) => {
        setForm((prev) => ({
            ...prev,
            ...
            <div className="w-full max-w-2xl">
                ...
                {!role && (
                    <div className="grid grid-cols-2 gap-4 w-full">
                        {(ONBOARDING_ROLES as RoleOption[]).map((r) => (
                            <button
                                key={r.value}
                                type="button"
                        ...
                        <div className="flex items-center justify-between bg-[#0f0f11] border border-white/10 rounded-2xl px-6 py-4">
                            <div className="flex items-center gap-3">
                                <span className="w-9 h-9 rounded-xl bg-amber-400/10 border border-amber-400/20 flex items-center justify-center text-base shrink-0">
                                    {
                                        (ONBOARDING_ROLES as RoleOption[]).find(
                                            (r) => r.value === role
                                        )?.icon
                                    }
                                </span>
                        ...
                        {role === "INTERVIEWER" && (
                            <div className="bg-[#0f0f11] border border-white/10 rounded-2xl p-8 flex flex-col gap-6">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {(YEARS_OPTIONS as YearOption[]).map((opt) => (
                                        <button
                                            key={opt.value}
                                            type="button"
                                            onClick={() =>
