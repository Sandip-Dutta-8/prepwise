...
import { getInterviewerProfile, type InterviewerProfile } from "@/actions/booking";
...
interface InterviewerProfilePageProps {
    params: Promise<{ id: string }>;
}

function ProfileHeroBackground() {
    return (
        <div className="absolute inset-0 -z-10 overflow-hidden" aria-hidden="true">
            ...
            />
            {/* primary amber glow, upper-left behind the avatar */}
            <div
                className="absolute -top-32 -left-24 w-lg h-lg rounded-full blur-[110px] opacity-[0.14]"
                style={{ background: "radial-gradient(circle, #fbbf24, transparent 70%)" }}
            />
            {/* secondary, dimmer glow, upper-right for balance */}
            <div
                className="absolute -top-40 right-0 w-md h-112 rounded-full blur-[120px] opacity-[0.08]"
                style={{ background: "radial-gradient(circle, #fbbf24, transparent 70%)" }}
            />
            {/* vignette so the edges settle back to solid black */}
            <div
                className="absolute inset-0"
                style={{
                    background:
                        "radial-gradient(ellipse at 20% 0%, transparent 0%, transparent 35%, #000 85%)",
                }}
            />
        </div>
    );
...
export default async function InterviewerProfilePage({
    params,
}: InterviewerProfilePageProps) {
    const { id } = await params;

    const user = await currentUser();
    if (!user) redirect("/");

    const dbUser: { role: UserRole; credits: number } | null =
    ...
    const interviewer: InterviewerProfile | null = await getInterviewerProfile(id);
