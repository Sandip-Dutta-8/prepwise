...
type PayoutStatus = "PROCESSING" | "PROCESSED";

interface PayoutReviewData {
    id: string;
    status: PayoutStatus;
...
interface PayoutReviewClientProps {
    payout: PayoutReviewData;
}

export default function PayoutReviewClient({
    payout,
}: PayoutReviewClientProps) {
        ...
        return (
            <div className="bg-[#0f0f11] border border-white/10 rounded-2xl p-10 flex flex-col items-center gap-3 text-center">
                ...
                <p className="text-xs text-stone-500 font-light">
                    {payout.interviewerName} · ${payout.netAmount.toFixed(2)}{" "}
    ...
    return (
        <div className="bg-[#0f0f11] border border-white/10 rounded-2xl p-8 flex flex-col gap-5">
            ...
            <Button
                variant="gold"
                disabled={!password.trim() || loading === true}
                onClick={() =>
                    approveFn({ payoutId: payout.id, adminPassword: password })
                }
                className="w-full"
